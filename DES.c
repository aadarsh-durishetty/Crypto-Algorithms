...
uint32_t FeistelPermutation(uint32_t data,
...
uint32_t feistel(uint32_t data, uint64_t subkey) {
  ...
  uint64_t expanded = FeistelPermutation(data, ExpansionPermutation, 32);
  // XOR with subkey
  expanded ^= subkey;
  // S-Box substitution
  uint32_t result = 0;
  for (int i = 0; i < 8; i++) {
    ...
    result |= (SboxEntries[i][row][col] & 0xF) << (28 - 4 * i);
  }
  // Permutation
  return FeistelPermutation(result, ExpansionPermutation, 32);
}

// Function to perform the initial permutation
uint64_t initialPermutation(uint64_t plaintext) {
  return permute(plaintext, InitialPermutation, 64);
}

// Function to perform the final permutation
uint64_t finalPermutation(uint64_t ciphertext) {
  return permute(ciphertext, FinalPermutation, 64);
}
...
void GenerateSubkeys(uint64_t key, uint64_t subkeys[16]) {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    subkeys[i] = permute(combined, SubkeyPermutation, 48);
  }
}

// Function to perform DES encryption
uint64_t Encryption(uint64_t plaintext, uint64_t key) {
  // Initial permutation
...
uint64_t Decryption(uint64_t ciphertext, uint64_t key) {
  // Initial permutation
...
int main() {
  uint64_t plaintext = 0x0123456722DEFABC;
  uint64_t key = 0x133457799BCCDFF1;
  ...
  // Decryption
  uint64_t decrypted = Decryption(ciphertext, key);
